...
- [ ] **Prompt caching (biggest cost/latency lever).** No `cache_control` anywhere —
      every turn re-sends system + tool schemas + history at full price. Add
      `cache_control: {type:'ephemeral'}` to the last stable system block (caches
      tools + system).
      - [ ] **Prerequisite:** move the **RAG context out of the system prompt**
        (`buildSystemPrompt` appends it) into the user turn / after the cache breakpoint —
        RAG changes per query and silently invalidates the cache otherwise.
      - Expected: ~90% cheaper on the cached prefix. Directly de-risks AI cost at scale.
